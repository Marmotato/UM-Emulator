...
%posici�n del �rea reflectiva (pared) usar para puntos especificos de
...
% %position of scatering particle
% x_s=4;
% y_s=4.5;
% z_s=2.5;
                            %par�metros de la simulaci�n%

Aw=1  ;                                     %�rea del elemento reflectante
pw=0.6 ;                                    %coeficiente de reflexi�n del �rea reflectiva
ang_rad = 60;                               %semi-�ngulo de mitad de potencia del LED
m= int32(-log(2)/log(abs(cos(ang_rad*pi/180))));   %n�mero Lambertiano
Ap=0.0001;                                  %�rea f�sica del receptor (1 cm^2)
eta=1.5;                                    %indice de refracci�n del PD
fov = 70;                                    %field of view

                  %par�metros de los �ngulos de inclinaci�n y rotaci�n%

beta_i=45;                                  %�ngulo de inclinaci�n del LED con respecto al eje z
alpha_i=45;                                 %�ngulo de rotaci�n del LED con respecto al eje x

beta_j=45;                                  %�ngulo de inclinaci�n del PD con respecto al eje z
alpha_j=45;                                 %�ngulo de rotaci�n del PD con respecto al eje x
...
time=0:Sampling_time:35e-9; % vector de tiempo donde se observar� el CIR
time=round(time);    %redondeo del tiempo 
...
%2st position of receiver
incidenciaw_radian12=incline(x_w1,y_w1,z_w1,x_j2,y_j2,z_j2,alpha_j,beta_j);
incidenciaw12=rad2deg(incidenciaw_radian12);%conversion de radianes a grados
...
incidenciaw13=rad2deg(incidenciaw_radian13);%conversion de radianes a grados
...
incidenciaw14=rad2deg(incidenciaw_radian14);%conversion de radianes a grados
...
incidenciaw15=rad2deg(incidenciaw_radian15);%conversion de radianes a grados

incidenciaw_radian25=incline(x_w2,y_w2,z_w2,x_j5,y_j5,z_j5,alpha_j,beta_j);
incidenciaw25=rad2deg(incidenciaw_radian25);

incidenciaw_radian35=incline(x_w,y_w,z_w,x_j4,y_j5,z_j5,alpha_j,beta_j);
...
% Calculo de la distancia euclidiana entre LED y PD para 5 puntos 
 ...
 %ind=round(dl1);      %redondeo del delay 
 ind=dl1
 index=find(ind==time); %encuentra el indice cuando el delay es igual a la funci�n de tiempo
...
%Figura del CIR del PD
%  figure
%  hold on
%  grid on
%  plot(time, h_led);
 
%Figura del CIR del LoS convolucionado con el CIR del PD
%  figure
%  hold on
%  grid on
%  plot(time, Final_responsel(1:length(time)));
 
 
 %-------- Calculo del CIR para la componente nLoS de una pared total fija en eje y------%

a=1; %indice para ir guardando componentes nLoS, delay, angulo de incidencia, angulo de inclinaci�n y angulo de rotaci�n, 
lx=6; ly=3; lz=5;  %Area del escenario Minero
Nx=lx*3; Ny=ly*3; Nz=lz*3 %round(lz*3); %numero de grid de cada superficie
...
for kk=1:Nx+1  
for ll=1:Nz+1
WP1=[x(kk) 0.2 z(ll)];    % ubicaci�n de la pared fija en y se mueven en x y z
r(a) = randi([0 90],1,1); % angulo de rotacion del elemento reflectante aleatorio uniforme
s(a) = randi([0 90],1,1); % angulo de inclinaci�n del elemento reflectante aleatorio uniforme
...
%tem=round(tem); %redondeo del vector de delays
...
var=round(var); %redonde el delay a 9 cifras significativas
...
%Figura del CIR del nLoS sin convolucionar
% figure
%  hold on
%  grid on
% plot(time, h_vector3(1:length(time)))

%Figura del CIR del nLoS convolucionado con el CIR del LED
Final_responsen=conv(h_led,h_vector3);
% figure
%  hold on
%  grid on
% plot(time, Final_responsen(1:length(time)))



%-------- Calculo del CIR para la componente nLoS de una pared total fija en eje y------%

b=1; %indice para ir guardando componentes nLoS, delay, angulo de incidencia, angulo de inclinaci�n y angulo de rotaci�n, 
...
for kk=1:Nx+1  
for ll=1:Nz+1
WP2=[x(kk) 3 z(ll)];    % ubicaci�n de la pared fija en y se mueven en x y z
r1(b) = randi([0 90],1,1); % angulo de rotacion del elemento reflectante aleatorio uniforme
s1(b) = randi([0 90],1,1); % angulo de inclinaci�n del elemento reflectante aleatorio uniforme
...
%tem1=round(tem1); %redondeo del vector de delays
...
var1=round(var1); %redonde el delay a 9 cifras significativas
...
figure
 hold on
 grid on
plot(time, h_vector1(1:length(time)))
...
figure
 hold on
 grid on
plot(time, Final_responsen1(1:length(time)))
...
temsca=zeros(1,length(time));
   %Funci�n que calcula el CIR y el delay del scatter
...
%Figura del CIR del scatter sin convolucionar
% figure
%  hold on
%  grid on
% plot(time, h_vector2(1:length(time)))

%Figura del CIR del scatter convolucionado con el CIR del LED
Final_responses=conv(h_led,h_vector2);
% figure
%  hold on
%  grid on
% plot(time, Final_responses(1:length(time)))

%Figura del CIR total convolucionado con el CIR del LED
 Final_response=Final_responsel+Final_responsen+Final_responsen1+Final_responses;
% figure
%  hold on
%  grid on
% plot(time, Final_response(1:length(time)))

%Figura en 3D del CIR total con respecto a D1 eje x=time, eje y= D1, eje z=CIR
 figure
 plot3(time,D1*ones(size(time)),Final_response(1:length(time)));
 grid on
 hold on
 
 %Figura en 3D del CIR LoS con respecto a D1 eje x=time, eje y= D1, eje z=CIR
 figure
 plot3(time,D1*ones(size(time)),Final_responsel(1:length(time)));
 grid on
 hold on
 
 %Figura en 3D del CIR nLoS con respecto a D1 eje x=time, eje y= D1, eje z=CIR
 figure
 plot3(time,D1*ones(size(time)),Final_responsen(1:length(time)));
 grid on
 hold on
 
 %Figura en 3D del CIR sca con respecto a D1 eje x=time, eje y= D1, eje z=CIR
 figure
 plot3(time,D1*ones(size(time)),Final_responses(1:length(time)));
 grid on
 hold on
 
 %Figura en 3D del CIR los+nlos con respecto a D1 eje x=time, eje y= D1, eje z=CIR
 figure
 plot3(time,D1*ones(size(time)),(Final_responsel(1:length(time))+Final_responsen(1:length(time))+Final_responsen1(1:length(time))));
 grid on
 hold on
 
 
%save('sce1sca80.mat','time','Final_responses')
