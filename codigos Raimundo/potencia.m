...
time=0:Sampling_time:45e-9; % vector de tiempo donde se observará el CIR
...
Nx=lx*3; Ny=ly*6; Nz=round(lz*3); %numero de grid de cada superficie
dA=lz*ly/(Ny*Nz); % area de cada grid
x=0:lx/Nx:lx; %vector que cobre todo el grid en x
y=0:ly/Ny:ly;  %vector que cobre todo el grid en y
z=0:lz/Nz:lz;  %vector que cobre todo el grid en z
...
incidencia_radian1(ii,jj)=incline(x_i,y_i,z_i,x(ii),y(jj),1.8,alpha_j,beta_j);
incidencia1(ii,jj)=rad2deg(incidencia_radian1(ii,jj)) ; %conversion de radianes a grados
...
%1st position of receiver
irradiancia_radian1(ii,jj)=rotacion(x(ii),y(jj),1.8,x_i,y_i,z_i,alpha_i,beta_i);
irradiancia1(ii,jj)=rad2deg(irradiancia_radian1(ii,jj)); %conversion de radianes a grados
% %2nd position of receiver
...
irradianciaw_radian=rotacion(x(ii),y(jj),1.8,x_w,y_w,z_w,alpha_w,beta_w);
irradianciaw=rad2deg(irradianciaw_radian);  %conversion from radian to degree

%angle de incidencia del enlace de pared a PD
%1st position of receiver
% incidenciaw_radian11=incline(x_w1,y_w1,z_w1,x(ii),y(jj),1.8,alpha_j,beta_j);
% incidenciaw11=rad2deg(incidenciaw_radian11);%conversion de radianes a grados
% 
% incidenciaw_radian21=incline(x_w2,y_w2,z_w2,x(ii),y(jj),1.8,alpha_j,beta_j);
% incidenciaw21=rad2deg(incidenciaw_radian21);

incidenciaw_radian31=incline(x_w,y_w,z_w,x(ii),y(jj),1.8,alpha_j,beta_j);
...
% Calculo de la distancia euclidiana entre LED y PD para 5 puntos 
  D1(ii,jj)= DIST(x_i,y_i,z_i,x(ii),y(jj),1.8);
...
%   D5= DIST(x_i,y_i,z_i,x_j5,y_j5,z_j5);
 ...
 %vector donde se guarda la CIR del LoS
                 %Función que calcula el CIR y el delay del LoS
 [m_HLoS1(ii,jj),dl1(ii,jj)]=HLoS_direct(x_i,y_i,z_i,x(ii),y(jj),1.8,Ap,eta,alpha_i,alpha_j,beta_i,beta_j,incidencia_radian1(ii,jj),incidencia1(ii,jj),m,fov,gv,fv,W,H,X,Y,t,es,c);
 
%  ind=round(dl1(ii,jj),9);%redondeo del delay a 9 cifras significativas
%  index=find(ind==time); %encuentra el indice cuando el delay es igual a la función de tiempo
%  h_vector(index)=h_vector(index)+m_HLoS1(ii,jj);  %coloca el CIR en el indice de delay correspondiente
...
%-------- Calculo del CIR para la componente nLoS de una pared total fija en eje y------%
 ...
 %indice para ir guardando componentes nLoS, delay, angulo de incidencia, angulo de inclinación y angulo de rotación, 

for kk=1:Nx+1  
for ll=1:Nz+1
WP1=[x(kk) 0.2 z(ll)];    % ubicación de la pared fija en y se mueven en x y z
r = randi([0 90],1,1); % angulo de rotacion del elemento reflectante aleatorio uniforme
s = randi([0 90],1,1); % angulo de inclinación del elemento reflectante aleatorio uniforme
incidenciaw_pru(ii,jj)=incline(x(kk),0.2,z(ll),x(ii),y(jj),1.8,alpha_j,beta_j); %angulo de radiancia del LED al elemento reflectante
incidenciawpru(ii,jj)=rad2deg(incidenciaw_pru(ii,jj));%conversion de radianes a grados
   %Función que calcula el CIR y el delay del nLoS
[m_HnLoSp,dnp] =HnLos_calculation(x_i,y_i,z_i,x(ii),y(jj),1.8,x(kk),0.2,z(ll),dA/70,pw,alpha_i,alpha_j,r,beta_i,beta_j,s,Ap,incidenciawpru(ii,jj),incidenciaw_pru(ii,jj),eta,m,fov,gv,fv,W,H,X,Y,t,es,c)  ;
re(kk,ll)=m_HnLoSp;
...
r1 = randi([0 90],1,1); % angulo de rotacion del elemento reflectante aleatorio uniforme
s1 = randi([0 90],1,1); % angulo de inclinación del elemento reflectante aleatorio uniforme
incidenciaw_pru1(ii,jj)=incline(x(kk),3,z(ll),x(ii),y(jj),1.8,alpha_j,beta_j); %angulo de radiancia del LED al elemento reflectante
incidenciawpru1(ii,jj)=rad2deg(incidenciaw_pru1(ii,jj));%conversion de radianes a grados
   %Función que calcula el CIR y el delay del nLoS
[m_HnLoSp1,dnp1] =HnLos_calculation(x_i,y_i,z_i,x(ii),y(jj),1.8,x(kk),3,z(ll),dA/70,pw,alpha_i,alpha_j,r1,beta_i,beta_j,s1,Ap,incidenciawpru1(ii,jj),incidenciaw_pru1(ii,jj),eta,m,fov,gv,fv,W,H,X,Y,t,es,c)  ;
...
[m_Hscat1,ds1]=H_scater(x_i,y_i,z_i,x(ii),y(jj),1.8,Ap,m,f,g,gymma,kr,km,ks,p,N,irradiancia1(ii,jj),c,alpha_i,beta_i)   ;
sc=sum(m_Hscat1);
Hsca(ii,jj)=sc;
a=a+1
end 
